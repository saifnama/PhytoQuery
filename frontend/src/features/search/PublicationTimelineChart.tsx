 ...
 *
 * Departures from the canonical shadcn example:
 *
 *   1. ``<ReferenceDot>`` marks the peak year.
 *   2. ``<Brush>`` stays MOUNTED with hover-revealed visibility
 *      so its internal start/end indices survive across hovers.
 *      Dragging either traveller narrows the visible year range (zoom).
 *   3. Mouse-wheel zoom while hovered — wheel-up narrows the window
 *      anchored to the cursor x-position, wheel-down widens it.
 *      Listener is attached non-passively via ``addEventListener`` so
 *      ``preventDefault()`` stops the page from scrolling while zooming.
 *   4. ``onYearClick`` callback allows clicking a data point to open
 *      the database explorer filtered to that year.
 *
 * Theme tokens flow from ``chartConfig.count.color = #00ACC1``,
 * which ``ChartContainer`` re-exports as ``--color-count``.
 */
      ...
      <ChartContainer config={chartConfig} className="h-full w-full">
        ...
        >
          ...
          {/* hover-revealed range slider (the zoom). Kept mounted so the
              ...
              notices "drag me to zoom" when it fades in. */}
          <Brush
            dataKey="name"
            height={22}
            stroke="var(--color-count)"
