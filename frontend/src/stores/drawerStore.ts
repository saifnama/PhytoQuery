 ...
 * notably SearchForm when source=Database is submitted from ExplorePage —
 * also need to ask the drawer to open with a query pre-applied as the
 * filter.
 *
 * Rather than lift Dashboard's complex drawer state up to ExplorePage
 * (which would force a structural restructure of Dashboard), we use
 * this tiny ephemeral store as a one-way signal:
 *
 *   - ExplorePage calls ``requestOpenWithQuery(query)`` when the user
...
interface DrawerState {
  /** Set by ExplorePage when the user submits the search bar with
