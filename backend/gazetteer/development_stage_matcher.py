"""
Dictionary-based Development Stage Matcher using spaCy PhraseMatcher.

Fast dictionary lookup for plant development stages - no heavy models needed.
...
class DevelopmentStageMatcher:
    """Fast dictionary matcher for development stages using spaCy PhraseMatcher."""
        ...
        if CACHE_FILE.exists():
            try:
                ...
                if ENTITY_TYPE in cache:
                    ...
                    logger.info(
                        f"[DevelopmentStageMatcher] Loaded {len(patterns)} patterns from cache"
                    )
                    return
    ...
    def _build_from_csv(self):
        ...
        CSV format:
        term,aliases
        flowering,flowering stage|flowering period
        early flowering,early bloom|beginning of flowering
        """
        csv_path = DATA_DIR / "development_stage.csv"
        ...
        logger.info(
            f"[DevelopmentStageMatcher] Built {len(patterns)} patterns from CSV (with aliases)"
        )
    ...
    def _save_cache(self, terms: List[str]):
        ...
        logger.info(f"[DevelopmentStageMatcher] Cache saved to {CACHE_FILE}")
    ...
    def match(self, text: str) -> List[Dict[str, Any]]:
        """Find all development stages in text.
        ...
        Returns:
            ...
            - span: matched text
            - type: entity type (DEVELOPMENT STAGE)
...
# Singleton instance
_matcher: Optional[DevelopmentStageMatcher] = None


def get_matcher() -> DevelopmentStageMatcher:
    """Get singleton matcher instance."""
    global _matcher
    if _matcher is None:
        _matcher = DevelopmentStageMatcher()
    return _matcher


def match_development_stages(text: str) -> List[Dict[str, Any]]:
    """Convenience function to match development stages in text.

    Args:
        text: Input text

    Returns:
        List of development stage entities
    """
...
if __name__ == "__main__":
    ...
    # Test
    test_text = """
    The study was conducted during the flowering stage.
    Early vegetative development was observed.
    Samples were collected at full bloom.
    """

    entities = match_development_stages(test_text)
    for e in entities:
